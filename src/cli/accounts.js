...
/**
 * Helper to get just the accounts array for the CLI
 * wraps the shared loadAccounts
 */
async function getAccountsConfig() {
  return await loadAccounts(ACCOUNT_CONFIG_PATH);
}

/**
 * Helper to save accounts using shared logic
 * Preserves settings and activeIndex from the loaded config
 */
async function persistAccounts(accounts) {
  // We need to load existing settings/index first to preserve them
  const { settings, activeIndex } = await getAccountsConfig();
