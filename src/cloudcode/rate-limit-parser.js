...
/**
 * Determine if a rate limit error is for a daily quota or short-term limit
 * @param {string} errorText - Error message text
 * @returns {'daily'|'rpm'} The type of limit detected
 */
export function parseLimitType(errorText) {
  const msg = (errorText || "").toLowerCase();

  // Daily limit indicators (Cloud Code API specific)
  if (
    msg.includes("daily") ||
  ...
  // Default to RPM (requests per minute)
  return "rpm";
}
...
export function parseResetTime(responseOrError, errorText = "") {
  ...
  if (!resetMs) {
    ...
    const rawMsg =
      (responseOrError instanceof Error
        ...
        : errorText) || "";
    const msg =
      rawMsg.length > MAX_ERROR_LENGTH
        ? rawMsg.substring(0, MAX_ERROR_LENGTH)
        : rawMsg;
    ...
    const quotaDelayMatch = msg.match(
      /quotaResetDelay[:\s"]+(\\d+(?:\\.\\d+)?)(ms|s)/i,
    );
    if (quotaDelayMatch) {
      ...
      logger.debug(
        `[CloudCode] Parsed quotaResetDelay from body: ${resetMs}ms`,
      );
    }
  ...
  if (resetMs !== null) {
    if (resetMs < MIN_WAIT_MS) {
      logger.debug(
        `[CloudCode] Reset time too small (${resetMs}ms), enforcing ${MIN_WAIT_MS}ms minimum`,
      );
      resetMs = MIN_WAIT_MS;
    }
  }
