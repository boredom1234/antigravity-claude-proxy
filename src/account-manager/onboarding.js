...
export async function onboardUser(
  ...
  logger.debug(
    `[Onboarding] Starting onboard with tierId: ${tierId}, projectId: ${projectId}, isFree: ${isFree}`,
  );
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        if (attempt < maxAttempts - 1) {
          logger.debug(
            `[Onboarding] onboardUser not complete, waiting ${delayMs}ms...`,
          );
          await sleep(delayMs);
        }
      } catch (error) {
        logger.warn(
          `[Onboarding] onboardUser error at ${endpoint}:`,
          error.message,
        );
        break; // Try next endpoint
      }
  ...
  logger.warn(
    `[Onboarding] All onboarding attempts failed for tierId: ${tierId}`,
  );
  return null;
}
