...
window.Components.accountManager = () => ({
  ...
  async toggleAccount(email, enabled) {
    ...
    try {
      ...
      if (data.status === "ok") {
        ...
        store.showToast(
          store.t("accountToggled", { email, status }),
          "success",
        );
  ...
  async exportAccounts() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        store.showToast(
          store.t("exportSuccess", { count: data.length }),
          "success",
        );
      } else if (data.error) {
  ...
  async importAccounts(event) {
    ...
    try {
      ...
      const accounts = Array.isArray(importData)
        ? importData
        : importData.accounts || [];
      ...
      if (data.status === "ok") {
        ...
        let msg =
          ...
          ` ${added.length} added, ${updated.length} updated`;
        if (failed.length > 0) {
