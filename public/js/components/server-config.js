...
window.Components.serverConfig = () => ({
  ...
  async setGeminiHeaderMode(mode) {
    ...
    this.serverConfig.geminiHeaderMode = mode;
    ...
    try {
      ...
      const data = await response.json();
      if (data.status === "ok") {
        store.showToast(`Gemini Header Mode set to ${mode}`, "success");
        await this.fetchServerConfig();
      } else {
        throw new Error(data.error || "Failed to update header mode");
      }
    } catch (e) {
      this.serverConfig.geminiHeaderMode = previousValue;
