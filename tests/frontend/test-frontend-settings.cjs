...
const BASE_URL =
  process.env.TEST_BASE_URL || `http://localhost:${process.env.PORT || 8672}`;
...
const tests = [
  ...
  },
  {
    ...
    async run() {
      ...
      if (missing.length > 0) {
        throw new Error(
          `Missing Claude config elements: ${missing.join(", ")}`
        );
      }
  ...
  },
  {
    ...
    async run() {
      ...
      return `Config has ${envKeys.length} env vars: ${envKeys
        .slice(0, 3)
        .join(", ")}${envKeys.length > 3 ? "..." : ""}`;
    },
  },
  {
