...
function analyzeContent(content) {
  ...
  const thinkingHasSignature = thinking.some(
    (t) => t.signature && t.signature.length >= 50
  );
...
function analyzeEvents(events) {
  return {
    ...
    signatureDeltas: events.filter(
      (e) => e.data?.delta?.type === "signature_delta"
    ).length,
    textDeltas: events.filter((e) => e.data?.delta?.type === "text_delta")
      .length,
    inputJsonDeltas: events.filter(
